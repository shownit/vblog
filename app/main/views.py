...
from ..auth.models import User, Role, Permission, Follow
...
def index():
    ...
    if show_followed:
        q = Post.query.join(Follow, Follow.followed_id == Post.author_id) \
            .filter(Follow.follower_id == current_user.id)
    else:
        q = Post.query
    pagination = q.order_by(Post.timestamp.desc()).paginate(page, per_page=20, error_out=False)
...
def post(id):
    ...
    pagination = post.comments.order_by(Comment.timestamp.desc()).paginate(page, 30, error_out=False)
...
def followers(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followers.paginate(page, per_page=50, error_out=False)
...
def followed_by(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followed.paginate(page, per_page=50, error_out=False)
